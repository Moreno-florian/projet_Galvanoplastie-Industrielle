...
class DaoCustomer
{
    ...
    public function addCustomer($newCustomer): void
    {

        $pdo = Dao::getConnection();     // Connection to the DB
        $request = $pdo->prepare("INSERT INTO client (Nom_client) VALUES (?)");    // Prepare the request to be execute

        try {
            $request->execute(array($newCustomer->get_customerName()));    // Execute the request
